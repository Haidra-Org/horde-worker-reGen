...
if [ "$hordelib" = true ]; then
 bin/micromamba run -r conda -n linux python -s -m pip uninstall -y hordelib horde-engine horde_sdk horde_model_reference
 bin/micromamba run -r conda -n linux python -s -m pip install horde-engine horde_model_reference --extra-index-url https://download.pytorch.org/whl/cu121
else
