...
python -s -m pip install torch==2.9.1 --index-url https://download.pytorch.org/whl/cu130 -U

if defined hordelib (
  python -s -m pip uninstall -y hordelib horde_engine horde_model_reference
  python -s -m pip install horde_engine horde_model_reference --extra-index-url https://download.pytorch.org/whl/cu130
) else (
  if defined scribe (
    python -s -m pip install -r requirements-scribe.txt
  ) else (
    python -s -m pip install -r requirements.txt --extra-index-url https://download.pytorch.org/whl/cu130 -U
  )
)
